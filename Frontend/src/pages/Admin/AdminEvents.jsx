...
export default function AdminEvents() {
  ...
  const handleDelete = async (e, id) => {
    e.stopPropagation();
    if (!window.confirm("WARNING: This will permanently delete the event. Are you sure?")) return;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      ...
      {/* EDITOR SECTION */}
      <div className={`bg-gray-900 border border-gray-800 rounded-xl p-6 shadow-2xl relative overflow-hidden transition-all duration-300 ${editingId ? 'ring-1 ring-orange-500' : 'ring-1 ring-green-600'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <button onClick={handleSubmit} className={`w-full py-3 rounded-lg font-bold text-white shadow-lg transition transform active:scale-95 ${editingId ? "bg-orange-600 hover:bg-orange-700" : "bg-green-600 hover:bg-green-700"}`}>
              {editingId ? "Update Details" : "Create Event"}
            </button>
          </div>
        </div>
      </div>

      {/* LIST SECTION: Existing Events */}
      <div className="mt-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {events.map(ev => (
            <div
              ...
              className={`group relative bg-gray-900 rounded-xl overflow-hidden border cursor-pointer hover:shadow-2xl transition-all duration-300 ${editingId === ev._id ? "border-orange-500 ring-2 ring-orange-500/50" : "border-gray-800 hover:border-indigo-500"}`}
            >
              <button
                onClick={(e) => handleDelete(e, ev._id)}
                className="absolute top-2 right-2 z-20 bg-red-600 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 hover:bg-red-700 transition-all shadow-lg transform hover:scale-110"
